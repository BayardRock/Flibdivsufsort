...
type SuffixArray(input: char[]) =
    // TODO: Use long arrays instead
    let TLen = sizeof<char> * input.Length
    ...
    member t.Search (searchString: string) = 
        let P = searchString.ToCharArray()
        let PLen = P.Length * sizeof<char>
        let numResults, idx = 
        ...
        [|
            for i = 0L to numResults - 1L do
                let strOffset = SA.[int (int64 idx + i)]
                yield strOffset
        |]
